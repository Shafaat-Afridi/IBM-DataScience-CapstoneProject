...
def get_scatter_chart(entered_site, payload_mass_selected):
    filtered_df = spacex_df[(spacex_df['Payload Mass (kg)']>=payload_mass_selected[0]) & (spacex_df['Payload Mass (kg)']<=payload_mass_selected[1])]
    if entered_site == 'ALL':
        fig = px.scatter(filtered_df, x='Payload Mass (kg)', y='class', color="Booster Version Category", title='Correlation between Payoad and Success for all Sites')       
        return fig
    else:
        fig = px.scatter(filtered_df[filtered_df['Launch Site']==entered_site], x='Payload Mass (kg)', y='class', color="Booster Version Category", title=f"Correlation between Payoad and Success for site {entered_site}")
        return fig
